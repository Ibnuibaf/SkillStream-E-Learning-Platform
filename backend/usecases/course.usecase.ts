...
class CourseUsecase {
  ...
  constructor(courseRepository: CourseRepository,communityRepository:CommunityRepository) {
  ...
  async createCourse(fields: ICourse,token:string) {
    ...
    try {
      ...
      const response =await this.communityRepository.createCommunity({course:res.data._id})
  ...
  async getInstructorCourses(query: any,token:string) {
    try {
      const { search } = query;
      const user=this.decodeToken(token)
      const res = await this.courseRepository.getInstructorCourses(search,user.id);
